...
def export_to_excel_report(sales_df, trxns_df, filename, unit_name, show_patrons=False):
    
    # Save chart images
    fig_pie = generate_pie_chart(sales_df.iloc[-1], "Grand Total Sales")
    sales_pie_img = figure_to_img_bytes(fig_pie)
    # plt.savefig("sales_pie.png", bbox_inches='tight')
    plt.close(fig_pie)

    fig_bar = plot_unit_channel_sales(sales_df.iloc[:-1], True, unit_name)
    sales_bar_img = figure_to_img_bytes(fig_bar)
    # plt.savefig("sales_bar.png", bbox_inches='tight')
    plt.close(fig_bar)

    if show_patrons:
        fig_pie_t = generate_pie_chart(trxns_df.iloc[-1], "Grand Total Transactions")
        trxn_pie_img = figure_to_img_bytes(fig_pie_t)
        # plt.savefig("trxns_pie.png", bbox_inches='tight')
        plt.close(fig_pie_t)
    
        fig_bar_t = plot_unit_channel_sales(trxns_df.iloc[:-1], False, unit_name)
